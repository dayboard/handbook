...
If and when the current company setup changes, we should address the above situations. For the purpose of this document, we interpret remote work as as a co-located employee of Dayboard working from home or from somewhere irregular.
...
##Policies
The majority of policies that support remote work in Dayboard apply to everyone at Dayboard, not just remote workers. The primary documents that outline these policies are:
...
Working remotely can add a financial burden on an individual. At the same time, it is also a privilege. While Dayboard is on its seed extension, Dayboard will subsidize co-working space costs for up to $10/day. Once Dayboard reaches profitability or raises its next major fundraising round, we will revisit this subsidy. -->
...
This is a time where employees can voice concerns about constraints that are limiting their ability to work effectively: this could be concerns about structures inside of Dayboard that limit their flexibility, concerns about how the way other teammates work, or anything else that might affect how they work. It's also a time where managers should give concrete feedback on whether an employee is upholding their responsibilities when they are working remotely.
...
This meeting is essential for debriefing what did and didn't work in the time they were gone and ensuring lessons learned are rolled back into the way we work at Dayboard.
